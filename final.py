...
if loaded_api_key_debug:
    print(f"DEBUG PRINT (final.py): GOOGLE_API_KEY loaded: {loaded_api_key_debug[:5]}...{loaded_api_key_debug[-5:]}")
else:
    print("DEBUG PRINT (final.py): GOOGLE_API_KEY NOT loaded from .env")
...
"""
    ...
    try:
        ...
        if status_placeholder: status_placeholder.write(f"Generating story content via Gemini ({gemini_model_to_use})...")
        logger.info(f"Generating story content with {gemini_model_to_use}...")
...
def rule_based_emotion(sentence):
    ...
    fear_words = ["scared", "afraid", "terrified", "fear", "frightened", "panic", "worried", "anxious", "dread", "horrified", "trembling", "hid under", "a little scared", "thunder", "storm", "lightning", "dark clouds", "hushed", "silenced"]
    if any(word in text for word in fear_words): return "sad"
    return None
...
def build_streamlit_ui():
        ...
        if not prompt_text_ui:
            ...
            with progress_area.status("🚀 Starting generation process...", expanded=True) as overall_status:
                try:
                    ...
                    prompt_parts.append("Please write a short, engaging children's story based on these elements.")
                    ...
                    logger.warning(f"Process stopped: {se}")
                except Exception as e_proc:
                    logger.error(f"Error in main processing block: {e_proc}", exc_info=True)
                    if 'overall_status' in locals() and overall_status: 
                        overall_status.update(label="❌ Processing Failed Critically", state="error", expanded=True)
                    st.exception(e_proc)
                finally:
    ...
    if st.session_state.generated_story_text:
        with story_text_area_expander:
            if not hasattr(story_text_area_expander, '_current_markdown') or \
               story_text_area_expander._current_markdown != st.session_state.generated_story_text:
                st.markdown(st.session_state.generated_story_text)
                story_text_area_expander._current_markdown = st.session_state.generated_story_text 
        if st.session_state.generated_story_text.strip(): 
            story_text_area_expander.expanded = True
...
# --- Guard for direct execution (when running streamlit run app.py) ---
